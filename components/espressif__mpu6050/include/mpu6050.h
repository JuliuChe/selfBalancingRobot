...
#define MAX_FIFO_BUFFER_SIZE         1024u        /*!< Maximum FIFO buffer size in bytes */
...
typedef enum {
    MPU6050_DLPF_BW_256HZ = 0, /*!< Digital Low Pass Filter bandwidth is 256Hz */
    MPU6050_DLPF_BW_188HZ  = 1, /*!< Digital Low Pass Filter bandwidth is 188Hz */
    MPU6050_DLPF_BW_98HZ   = 2, /*!< Digital Low Pass Filter bandwidth is 98Hz */
    MPU6050_DLPF_BW_42HZ   = 3, /*!< Digital Low Pass Filter bandwidth is 42Hz */
    MPU6050_DLPF_BW_20HZ   = 4, /*!< Digital Low Pass Filter bandwidth is 20Hz */
    MPU6050_DLPF_BW_10HZ   = 5, /*!< Digital Low Pass Filter bandwidth is 10Hz */
    MPU6050_DLPF_BW_5HZ    = 6, /*!< Digital Low Pass Filter bandwidth is 5Hz */
    MPU6050_DLPF_BW_NONE   = 7, /*!< No filter applied */
} mpu6050_dlpf_bw_t;
...
typedef struct{
    ...
    uint8_t smplrt_div;
    mpu6050_dlpf_bw_t dlpf_cfg;
}mpu6050_sampling_config_t; //Sampling frequency divider
...
/**
 * @brief Set accelerometer and gyroscope Sampling frequency and filter according to the configuration
 *
 * @param sensor object handle of mpu6050
 * @param config pointer to mpu6050_sampling_config_t structure containing config->smplrt (0-255), config->dlpf_cfg (0-7)
 *  config->sampling_freq (Hz) is calculated from config->smplrt_div and config->dlpf_cfg for future use.
 * @return
 *     - ESP_OK Success
 *     - ESP_FAIL Fail
 */
...
/**
 * @brief Get gyroscope calibration values
 *
 * @param gyro_calibrations gyroscope values to be filled
 *
 * @return
 *     - ESP_OK Success
 *     - ESP_FAIL Fail
...
/**
 * @brief Configure and enable the FIFO stack of mpu6050.
 *
 * @param sensor object handle of mpu6050
 * @param fifo_sources set sensor sources to be stored in FIFO stack :
 *                  (Bit0) - MPU6050_FIFO_EN_SLV0_BIT - Slave 0 registers EXT_SENS_DATA written into FIFO buffer
 *                  (Bit1) - MPU6050_FIFO_EN_SLV1_BIT - Slave 1 registers EXT_SENS_DATA written into FIFO buffer
 *                  (Bit2) - MPU6050_FIFO_EN_SLV2_BIT - Slave 2 registers EXT_SENS_DATA written into FIFO buffer
 *                  (Bit3) - MPU6050_FIFO_EN_ACCEL_BIT - All registers for the accelerometer written into FIFO buffer
 *                  (Bit4) - MPU6050_FIFO_EN_Z_GYRO_BIT - Registers for the Z axis of gyrosocpe can be written into FIFO buffer
 *                  (Bit5) - MPU6050_FIFO_EN_Y_GYRO_BIT - Registers for the Y axis of gyrosocpe can be written into FIFO buffer
 *                  (Bit6) - MPU6050_FIFO_EN_X_GYRO_BIT - Registers for the X axis of gyrosocpe can be written into FIFO buffer
 *                  (Bit7) - MPU6050_FIFO_EN_TEMP_BIT - Temperature register written into FIFO buffer
 * 
 * @return
 *      - ESP_OK Success
 *      - ESP_ERR_INVALID_ARG A parameter is NULL or incorrect
...
/**
 * @brief Reset the FIFO stack (for the case of a fifo overflow interupt).
 *
 * @param sensor object handle of mpu6050
 * @param fifo_count[out] pointer to store the number of bytes in the FIFO stack
 *
 * @return
 *      - ESP_OK Success
 *      - ESP_ERR_INVALID_ARG A parameter is NULL or not valid
 *      - ESP_FAIL Failed to reset FIFO stack on mpu6050
 */
 ...
 *      - ESP_FAIL Failed to reset FIFO stack on mpu6050
 */
extern esp_err_t mpu6050_reset_fifo_stack(mpu6050_handle_t sensor);


/**
 * @brief Read the FIFO stack (for the case of a data ready interupt).
 *
 * @param sensor object handle of mpu6050
 * @param fifo_data[out] buffer to store the FIFO data read from mpu6050
 * @param max_len maximum length of the fifo_data buffer
 * @param actual_len[out] actual length of the data read from FIFO stack
 *
 * @return
 *      - ESP_OK Success
 *      - ESP_ERR_INVALID_ARG A parameter is NULL or not valid
...
/**
 * @brief Calibrate gyroscope values on startup
 *
 * @param sensor object handle of mpu6050
 *
 * @return
